...
class Database
{
	...
	public function GetContent($table_name, $where_check = array(), $custom = "", $limit = null)
	{
		$value_push = array();
		$where = "";

		if ($where_check !== null && count($where_check) != 0) {
			$where = " WHERE ";
			foreach ($where_check as $key => $value) {
				$where .= $key . " = ? AND ";
				array_push($value_push, $value);
		...
		$sql = "SELECT * FROM " . $table_name . $where . " " . $custom . $limit_clause;
